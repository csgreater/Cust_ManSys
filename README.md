...
> 本文按当前工作区代码整理，最后核对日期为 2026-07-17。数据库的实际数据、环境变量和线上部署状态未在本次文档核对中验证。

## 当前完成情况

| 模块 | 当前状态 | 已有能力 |
| --- | --- | --- |
| 账户与权限 | 已实现 | 登录会话、用户/角色、按部门/平台/店铺的数据范围隔离；订单中的姓名、手机和地址在服务端页面会脱敏显示。 |
| 订单管理 | 已实现 | 多条件查询、默认上月时间范围、订单明细查看和 Excel 导出。 |
| Excel 导入 | 已实现 | 模板下载、`.xlsx`/`.xlsm` 上传、后台解析校验、进度轮询、异常行查看、确认入库和整批文件指纹去重。 |
| 数据看板 | 已实现 | 首页概览、产品分析、店铺分析及电商经营驾驶舱；支持销售额、利润、利润率、订单数等指标。 |
...
- 必填字段：客户编号、渠道分类、渠道平台、销售渠道、货品编号、销售额、成本金额、利润。
- `订单来源`、`客户名称`、地址、省/市/区县、`货品分类` 等字段在模板中可用；货品分类为可选。
- 数值空值按 `0` 处理，允许负数；同一订单同一货品允许多行。
- 导入文件采用整批数据指纹避免重复入库；订单明细不提供物理删除功能。
