...
基于 **FastAPI + MySQL + Vue 3/Vite** 的经营异常与分析报告工作台。主要流程是导入数据、发现异常、核查证据、生成和导出报告；订单明细作为计算底层保留。

> 最后核对日期为 2026-09-08。异常和报告主流程已完成本地验证，开发库已完成新增结构迁移。源码与对应的前端构建统一交付；交付范围、验证结果与升级步骤见 [异常与报告交付说明](docs/workbench-release-2026-09-08.md)。生产部署尚未执行。
...
| 异常中心 | 本地已实现 | 数据质量/经营异常分类、参考阈值、完整分页与 XLSX、证据查看、待核查/确认/排除和备注。普通订单退出主导航。 |
| 分析报告 | 本地已实现 | 月度经营/异常专题报告、完整范围计算、不可变版本、重算、当前权限校验、PDF/XLSX。模型未配置也可生成。 |
| Excel 导入 | 已实现 | 模板、后台分块校验、完整错误报告、归属及行范围检查、确认入库、整批指纹去重；中断解析恢复与新增来源批次/行号。 |
| 数据看板 | 已实现 | 数据中心首页看板、产品分析、店铺分析及多维经营驾驶舱；包含核心 KPI、同期比较、经营趋势、平台结构、TOP 产品和风险信号。 |
| 智能分析 | 已实现 | Ark/规则混合解析完整经营意图，支持权限内实体匹配、条件纠偏、周期对比和 SSE 实时过程；模型不生成 SQL，查询统一经过模板编译与安全审计。 |
| 前端 | 已实现 | Vue 3/Vite 单页运营台；构建后的 `frontend/dist` 由 FastAPI 挂载在 `/ui/`，保留 Jinja 页面作为兼容入口。 |
| 健康检查 | 已实现 | `/healthz` 检查进程，`/readyz` 检查数据库连接。 |
| Coding Plan | 已实现，待上线验收 | 提供 `/coding-plan` 和 Ark 对话接口；仅登录用户可访问，包含输入长度、按用户/IP 限流、每日额度、调用审计和生产配置校验。真实模型调用和生产环境仍需单独验收。 |
| 质量基线 | 已实现 | 单元测试覆盖数据口径、导入、权限、异常、报告和前端会话；CI 执行 Python 编译、单测、自然语言自检、前端测试和构建。另有独立合成 MySQL 集成脚本。 |

当前异常与报告改动已完成本地审查和验证；后续维护方向见 [维护更新计划](docs/maintenance-plan.md)，其中历史工作区状态以本次交付说明为准。
...
使用 Python 3.10+；本轮在 Python 3.13.4、MySQL 8.0.42 验证，CI 使用 Python 3.11。复制并填写环境变量：
...
- 异常中心优先展示需要核查的对象。参考阈值默认：低利润率 5%、扣减费用率 20%、下降 20%、最低收入 1000；管理员可修改，版本写入报告。负利润不受最低收入限制。
- 费用口径为运费、辅料、分摊费用之和；快递费/物流费重叠口径尚未确认，不重复计入。
- 整月范围对比前一个同长度自然月范围，非整月对比等长前期；可选去年同期。没有本期数据或可用基线时明确提示，不据此认定经营下降。
- 质量异常不按发货日期剔除，以保留无效日期记录；仍受其他筛选、批次归属和数据范围限制。
- 报告和核查记录按账号保存。重算创建新版本，历史版本保留；权限收窄后不能读超范围历史内容。数据、规则或核查备注改变后可重新生成。
- 普通导入员仅操作本人且当前范围合法的批次，管理员承担集中导入管理。历史已入库数据没有可靠来源记录时，不自动补猜批次。

- 所有查询、看板、分析和导出均受角色数据范围限制。
- 首页看板默认查询上月；日期范围、部门、平台和店铺筛选仅在点击“执行筛选”后请求，避免输入过程反复扫描数据库。
- 智能分析会把问题解析为 `AnalysisIntentV2`，再由服务端匹配当前用户有权访问的真实平台、店铺、部门、地区、分类和产品值；无法唯一确认的条件会暂停并要求用户纠偏。
- 模型只输出结构化意图，不能生成或执行 SQL。服务端使用参数化只读模板，强制带日期和权限范围；单次分析最长三年、最多执行四个查询、单个查询最多返回 200 行。
- `POST /api/analytics/stream` 以 SSE 返回理解需求、解析条件、权限校验、生成计划、读取数据和形成结论等真实阶段；`/api/analytics/ask` 与 `/api/analytics/parse` 保持同步调用兼容。
- `ANALYTICS_API_TOKEN` 可为分析 API 配置服务访问令牌；生产环境应设置为至少 24 位的随机值。

### 低配置服务器说明

- 首页比较周期由统一函数确定，不因是否使用月汇总而改变；省市、来源及 SKU 等汇总不支持的条件回退到明细查询。
- 看板趋势最多按日展示 62 天，更长范围自动按月汇总；前端不引入大型图表库，体积以本次构建输出为准。
- 异常和报告从完整授权分组计算，分页不影响总量；全量分组及导出仍消耗内存，生产大容量测试尚未完成，不应把合成小样本验证当作容量验收。
- 导出使用服务端游标按 500 行分批生成 CSV，不把全部结果一次性放入内存，并对可能触发电子表格公式的文本做转义。
- 导入状态轮询采用逐步退避，并在离开导入页面时取消；前端会取消过期请求并复用相同筛选条件的结果。

### 可选 Ark 能力

系统默认使用增强本地规则，并在界面标注“规则降级模式”。要启用 Ark 的完整结构化意图解析和诊断类结论整理，在服务端 `.env` 中配置：
...
本轮新增结构使用 `python scripts/migrate_anomaly_workbench.py`，随后运行 `python scripts/check_db_ready.py`。不要在已有库上使用初始化脚本升级，因为初始化还会处理默认角色配置。开发与测试依赖安装使用 `pip install -r requirements-dev.txt`。
